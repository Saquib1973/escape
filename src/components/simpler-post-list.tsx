...
const SimplerPostList: React.FC<Props> = ({ posts, emptyText = 'No posts yet', hrefFor }) => {
        ...
        <Link href={hrefFor(post)} key={post.id}>
          <div className="p-4 bg-dark-gray-2">
            <div className="flex items-center gap-4">
              ...
              {post.isSpoiler ? (
                <span className="ml-auto text-yellow-400 text-sm">spoiler alert</span>
              ) : null}
            </div>
            <div className="mt-1.5 flex gap-3 text-gray-400 text-xs">
              ...
              {post.rating != null ? <span>Rating: {post.rating}/10</span> : null}
            </div>
          </div>
        </Link>
      ))}
